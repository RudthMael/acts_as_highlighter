...
* p2 - не обязательный параметр - вариант синтаксиса ( см. ниже: _Варианты_ _синтаксиса_ ), если он не указан, то установлен в значение по умолчанию ( см. ниже: _Синтаксис_ _по-умолчанию_ ), _строка_
* p3 - не обязательный параметр - хеш опций ( см. ниже: _Хеш_ _опций_ ), если не установлен - то все опции по-умолчанию, _хеш_
...
[:brush]        Вариант синтаксиса, его можно передать и через хеш-опций и как параметр перед хешом опций.
                Если не установлен - то это синтаксис по-умолчанию ( см. ниже: Синтаксис по-умолчанию ).
                Примеры:
...
[:class_name]    Значение по-умолчанию: ''. Задать элементу css-класс для обработки его (элемента) через стили css. Пример:
...
[:tab_size]      Значение по-умолчанию: "4". Размер табыляции.

[:toolbar]       Значение по-умолчанию: "true". Показывать малюсенькую ссылочку в виде знака вопрос справа вверху - при нажатии на которую вылетает копирайт Алекса Горбачёва.
